...
before_task_start ALL => sub { $before_task_start_all++ };
before_task_start foo => sub { $before_task_start++ };
before ALL => sub { $before_all++ };
before foo => sub { $before++ };

after foo => sub { $after++ };
after ALL => sub { $after_all++ };
after_task_finished foo => sub { $after_task_finished++ };
after_task_finished ALL => sub { $after_task_finished_all++ };
